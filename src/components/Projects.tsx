...
const Projects = () => {
    ...
    return (
        <div id="projects" className="relative flex flex-col items-center justify-center gap-6 z-10 py-20 md:py-40 overflow-hidden bg-gradient-to-b from-sky-800 to-black">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} viewport={{ once: true }} className="flex gap-2 z-10 py-2">
                {filterTags.map((tag) => (
                    <button key={tag} className={`rounded-full border-2 border-[#9BF6FF] text-[#9BF6FF] font-bold select-none w-fit flex gap-1 py-2 px-4 transition-all duration-300 ${ selectedTag === tag ? 'bg-[#9BF6FF] text-black shadow-lg shadow-[#9BF6FF]/25' : 'bg-transparent hover:bg-[#9BF6FF]/10' }`} onClick={() => setSelectedTag(tag)}>
                        {tag}
                    </button>
                ))}
            </motion.div>

            <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 gap-[50px] z-10 mt-8 items-start">
                <AnimatePresence>
                    {filteredProjects.slice(0, visibleCount).map((project) => (
                        <motion.div key={project.name} variants={itemVariants} layout>
                            <ProjectCard {...project} />
                        </motion.div>
                    ))}
                </AnimatePresence>
