            ...
            <AnimatePresence>
                {contactFormOpen && (
                    <motion.div
                        ...
                        exit={{opacity: 0}}
                        transition={{duration: 0.5}}
                        className={"fixed inset-0 bg-black/50 background-blur-lg z-50 flex items-center justify-center p-4"}
                    >
                        <motion.div
                            ...
                            <form className={"space-y-4"}>
                                <div>
                                    ...
                                    <input type={"text"} id={"name"} placeholder={"Your Name"} className={"w-full px-4 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 bg-gray-700 "}/>
                                </div>
                                <div>
                                    <label htmlFor={"email"} className={"block text-sm font-medium text-gray-300 mb-1"}>
                                        Email
                                    </label>
                                    <input type={"email"} id={"email"} placeholder={"Your Email"} className={"w-full px-4 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 bg-gray-700 "}/>
                                </div>
                                <div>
                                    <label htmlFor={"message"} className={"block text-sm font-medium text-gray-300 mb-1"}>
                                        Message
                                    </label>
                                    <textarea rows={4} id={"message"} placeholder={"How may I help you?"} className={"w-full px-4 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 bg-gray-700 "}/>
                                </div>
                                <motion.button
                                    type={"submit"}
                                    whileHover={{scale:1.03 }}
                                    whileTap={{scale: 0.97 }}
                                    className={"w-full px-4 py-2 bg-gradient-to-r from-cyan-600 to-cyan-400 hover:from-cyan-700 hover:to-blue-700 transition-all duration-300 rounded-lg shadow-md hover:shadow-lg hover:shadow-cyan-600/50"}>
                                    Send Message
                                </motion.button>
                            </form>
                        </motion.div>
